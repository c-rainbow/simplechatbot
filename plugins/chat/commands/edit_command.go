...
var _ ChatCommandPlugin = (*EditCommandPlugin)(nil)
...
func (plugin *EditCommandPlugin) read(commandName string, channel string, sender *twitch_irc.User,
	...
	// Make sure that the plugin type is EditCommandPluginType
	if command.PluginType != EditCommandPluginType {
...
func (plugin *EditCommandPlugin) action(
	...
	err = plugin.repo.EditCommand(channel, parsedCommand)
	if err != nil {
		// TODO: pull failure message for
		// failureResponse, exists := command.Responses[EditFailureKey]
...
func (plugin *EditCommandPlugin) output(channel string, toSay string, err error) error {
...
func (plugin *EditCommandPlugin) userHasPermission(
